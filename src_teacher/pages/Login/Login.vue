        ...
        <form>
          <div :class="{on: isShowSms}">
            ...
            <section class="login_verification">
              <input type="tel" maxlength="8" placeholder="验证码"  v-model="phoneCode" name="phoneCode" v-validate="'required|phoneCode'">
          ...
          <div :class="{on: !isShowSms}">
            <section>
              <section class="login_message">
                <input type="tel" maxlength="11" placeholder="手机/邮箱/用户名" v-model="email" name="email" v-validate="'required|email'">
                <span style="color: red;" v-show="errors.has('email')">{{ errors.first('email') }}</span>
              </section>
              <section class="login_verification">
                <input :type="isShowPwd ? 'text' : 'password'" maxlength="8" placeholder="密码" v-model="password" name="password" v-validate="'required|password'">
              ...
              <section class="login_message">
                <input type="text" maxlength="11" placeholder="验证码" v-model="picCode" name="picCode" v-validate="'required|picCode'">
                <span style="color: red;" v-show="errors.has('picCode')">{{ errors.first('picCode') }}</span>
    ...
    methods: {
      ...
      async login(){
        if(this.isShowSms){
          ...
          const success = await this.$validator.validateAll(['email', 'password', 'picCode']) // 对指定的所有表单项进行验证
        }
      }
    }
  }
